...
public class MeshCreator : MonoBehaviour
{
    ...
    public void CreateUnderground(List<Vector3> genCurvePoints, int chunkCount = -1, int underground_height = -1)
    {
        ...
        if (chunkCount <= 0) { chunkCount = Mathf.FloorToInt(genCurvePoints.Count/4); }
        if (underground_height <= 0) { underground_height = undergroundHeight; }

        int chunkSize = Mathf.FloorToInt(genCurvePoints.Count / chunkCount); //get number of chunks based on chunk size

        Debug.Log("Underground Mesh Creator :: chunkCount " + chunkCount + " // underground_height " + underground_height + " // chunkSize: "  + chunkSize);
        ...
        undergroundMesh.Clear();
        undergroundMesh.vertices = GetVertices(genCurvePoints, underground_height, chunkSize, chunkCount);
    ...
    public Vector3[] GetVertices(List<Vector3> genCurvePoints, int underground_height, int triSize, int chunkCount)
    {
        ...
        for (int i = 1; i < chunkCount; i++)
        {
            ...
            Vector3 topVertice = new Vector3(genCurvePoints[nextHorzPointIndex].x, genCurvePoints[0].y, 0); // 2
